...
public:
	...
	hvec2f(const point& p); // shuffle
	hvec2f(const __m128& x, const __m128& y);
	hvec2f(const scalar& x = 0, const scalar& y = 0);
	scalar operator[](const size_t i) const;
	scalar& operator[](const size_t i);

	//protected:

	_MM_ALIGN16 std::array< _MM_ALIGN16 __m128, 2 > _vS;
