...
// A small RNG just used to initialize the higher-quality generator below.
...
xoroshiro128plus::xoroshiro128plus(uint64_t initialSeed)
{
	s[0] = splitmix64(initialSeed);
	s[1] = splitmix64(s[0]);
}

// vital operation for xoroshiro random generator
static inline uint64_t rotl(const uint64_t x, unsigned k) {
	return (x << k) | (x >> (64 - k));
}
